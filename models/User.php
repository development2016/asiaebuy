...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public static function checkMenu($modul)
    {
        ...
        WHERE acl.user_id = '".$user_id."' AND acl_menu.menu_id = '".$modul."'");
        $data = $sql->queryAll();
        /*$sql = $connection->createCommand("SELECT acl.menu,lookup_menu.menu,role.role_id,role.user_id FROM acl 
            LEFT JOIN role ON acl.role = role.id
            LEFT JOIN lookup_menu ON acl.menu = lookup_menu.id
            WHERE role.user_id = '".$user_id."' AND acl.menu = '".$modul."'");
        $data = $sql->queryAll(); */
